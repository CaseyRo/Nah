...
# People mode (CDI-1879): HARNESS_ARGS="--degree 150" PROBE="/feed?person=p0000&limit=30"
set -uo pipefail
name=$1; port=$2; workdir=$3; start=$4; reset=$5; artifact=$6; dbglob=$7
probe=${PROBE:-/feed?circle_id=fam000&limit=1}; hargs=${HARNESS_ARGS:-}
...
wait_up() {
  for _ in $(seq 1 600); do
    curl -sf -o /dev/null "http://127.0.0.1:$port$probe" && return 0
    sleep 0.1
  done
...
python3 "$here/harness.py" --port "$port" $hargs seed --concurrency 16 > "$out/$name.seed.json" 2>/dev/null
stop_server

echo "== $name: cold start against a seeded database =="
python3 "$here/harness.py" --port "$port" $hargs coldstart --cmd "$start" --path "$probe" > "$out/$name.cold.json" 2>/dev/null
sleep 1
...
mpid=$!
python3 "$here/harness.py" --port "$port" $hargs bench --concurrency 1  --requests 1000  > "$out/$name.b1.json" 2>/dev/null
python3 "$here/harness.py" --port "$port" $hargs bench --concurrency 50 --requests 50000 > "$out/$name.b50.json" 2>/dev/null
# Every file a server keeps open is a descriptor; per-person files make that a number worth having.
fds=$(lsof -p "$srvpid" 2>/dev/null | wc -l | tr -d ' ')
kill $mpid 2>/dev/null
stop_server

dbbytes=$(du -sk $dbglob 2>/dev/null | awk "{s+=\$1} END {print s*1024}")
artbytes=$(du -sk "$artifact" 2>/dev/null | awk "{print \$1*1024}")
python3 - "$out/$name.json" "$name" "${idle:-0}" "${artbytes:-0}" "${dbbytes:-0}" "$out" "${fds:-0}" <<PYEOF
import json, sys
res, name, idle, art, db, out, fds = sys.argv[1:]
def load(p):
...
d = {"candidate": name,
     ...
     "open_files_after_bench": int(fds)}
