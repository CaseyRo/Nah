...
On 2026-09-15 music came back as a fourth type. M1 posts and reads text moments. This restatement specifies all four types, what a moment may carry, how it is deleted, and how the feed and a person's page show it, so M3 builds each type once.
...
- **feed**: a card per type, the timeline clock, a sentence in place of any moment the app cannot show, a marker where a moment was deleted, and an end to the page. A song opens in the music service the reader picks, and nothing is fetched from music services before that tap.
...
- **Elsewhere**: cap-01's + offers four items, and its no-reporting rule names the place lookup and the song-link lookup as exceptions. nah-vision gains the principle that Nah? keeps no record of where anyone is. cap-06 is removed, because music was the part of it worth keeping.
...
- Linear: CDI-1843 to CDI-1847 cover photo, voice and the queue. Music, deleting a moment, the short line, the place and the person page have no issue yet.
- `docs/decisions`: notes on ADR-0006, ADR-0008 and ADR-0016, so the codebase wiki needs recompiling.
