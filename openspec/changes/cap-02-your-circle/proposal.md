...
- **joining**: an invitation registers a person and connects them to whoever invited them, in one step. The first person on a server uses an operator's invitation. An invitation survives installing the app. The first run follows ADR-0005's ritual and shows only real moments.
- **connecting**:
  ...
  - refusals are said in a sentence that names whose circle is full, never with a number
  - a directory finds people without learning who is connected to whom
...
- **Moved here from cap-01**: onboarding and the full-circle message.
...
- `connecting`: the touch, the link, the life of an invitation, refusals, and the directory.
...
- `apps/server`: a person's invitations become single-use and revocable, and still never expire. A route ends a connection. The refusal for a full circle names whose it is. The directory is new and runs centrally (ADR-0010).
...
- Linear: CDI-1839 asks for short-lived links and CDI-1842 for expiry. Both predate this ruling and need rewording. CDI-1842 also leaves the full-circle sentence open, and the ruling is to name whose circle it is.
