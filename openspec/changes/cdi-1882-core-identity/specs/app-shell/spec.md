...
The app SHALL keep the last feed it loaded on the device and show it at launch, before any network response arrives and when none arrives. It SHALL keep that copy in the app's private storage, under the phone's own encryption, and SHALL exclude it from device backups (ruled 2026-09-15). When the server cannot be reached, the app SHALL say so in a sentence and keep showing what it has.
...
#### Scenario: A phone backup

- **WHEN** the phone is backed up to iCloud or to Google
- **THEN** the stored feed is not in the backup
