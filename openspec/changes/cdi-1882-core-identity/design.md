...
This document records only the decisions that shape CDI-1882. Values live in `DESIGN.md`; reasons live in `docs/decisions/`.
...
### No profile header, one glass surface

Ruled 2026-09-15. A person's page starts with an avatar and a name, with no cover photo (CDI-1884). The compressing profile header goes, and with it the second glassmorphism surface; the radial menu's backdrop is the only one left in `DESIGN.md`.

### The stored feed stays on the phone

Ruled 2026-09-15. Once moments are sealed, the copy of the feed kept for opening offline is unsealed. It lives in the app's private storage under the phone's own encryption, and is excluded from iCloud and Google backups, as the device key already is.

### The bundle identifier stays works.cdit.nah

Ruled 2026-09-15. Nah? ships as a CDIT product under CDIT's developer accounts. The identifier becomes permanent at the first store submission, and push credentials and universal links (CDI-1837) are tied to it.
...
| Four-step onboarding and the "150 friends max" explainer | CDI-1883, as joining, following ADR-0005's ritual. |
| Friend-limit dialog and inactive-friend suggestions | CDI-1883. A full circle is said in words, naming whose it is (ADR-0004), with no suggestions. |
| Reaction icons and the heart button | CDI-1885, decided by ADR-0018. |
...
| Music, location and status moments | Music is a moment type again (ADR-0006, amended 2026-09-15); location and status stay out. CDI-1884 owns moment types. |
| Moment card variants, timeline clock, end of feed | CDI-1884. |
| Offline queue and media cache | CDI-1884 (CDI-1847, M3). |
