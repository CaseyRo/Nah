# ~~PWA Shell Spec~~ (Superseded)

> **Superseded 2026-05-19** by [`app-shell`](../app-shell/spec.md) per [ADR-0001 — Flutter over PWA](../../../../../docs/decisions/0001-flutter-over-pwa.md).
>
> Kept in the repo as the historical record. The requirements below describe the SvelteKit PWA shell we *would have* shipped under the original 2026-01 framing. The Flutter pivot replaced this capability with `app-shell`, which covers the same surface area (install, splash, offline cache, push, status bar) using native platform conventions instead of web APIs.
>
> Do not implement against this spec. It exists so the evolution is visible.
...
| Date | Change | Why |
|---|---|---|
| 2026-01-28 | Spec created as part of `cap-01-core-identity`, alongside SvelteKit PWA framing | Original "act small, MVP = PWA" decision (now also revised) |
| 2026-05-19 | Spec superseded by [`app-shell`](../app-shell/spec.md) | Flutter pivot per [ADR-0001](../../../../../docs/decisions/0001-flutter-over-pwa.md). PWA capability replaced with native-app capability. |
