...
*(Rewritten 2026-09-16.)* A 48px analog clock face fixed in the feed's time lane, its centre on the midpoint of the feed's visible area between the header's hairline and the safe area (460pt from the top of a 390 by 844 phone; proposed, the screen's own middle at 422pt being the alternative), so it runs with you as you scroll and the lane's labels pass underneath it; it never floats over content, only over the lane. Its hands read the time at that line, interpolated between the moment just above and the moment just below, so scrolling through the space between two moments turns the hands through the time between them; on a label they read that moment exactly. Across a gap of up to twelve hours the hands turn through the true time between the two moments; across a longer gap they turn once round the dial and the date changes at the midpoint of the gap (proposed). The face is Surface 1 with a 1px Surface 4 stroke, four ticks in Text 4 at 1.5px, the hour hand 3px and 11px long and the minute hand 1.5px and 16px long in Text 1 (the weight difference is what tells them apart at this size), the second hand 1.25px and 18px long with a 4px tail through the pivot, and a 3.5px centre dot, both in Pomegranate. The second hand sweeps at most one full turn between one moment and the next, and runs in real time while the clock holds for a pull to refresh. The date ("Sat 13 Sep") sits centred below the face in 12px/500 Text 2 and crossfades in 150ms when the time the hands read crosses midnight. In light mode the face carries the Clock shadow; in dark mode there is no shadow and the stroke is the border. Fades in over 150ms on scroll-start, fades out over 250ms after 2 seconds of scroll inactivity, and is never shown on a still feed. On pull to refresh the clock rides to the top of the lane inside the 72pt gap, reading now in real time, holds there while loading, and fades over 250ms as new moments settle in on the settle spring (400ms); the gap closes in 250ms when nothing new arrived. Under `prefers-reduced-motion` there is no face: the lane label of the moment nearest the middle is restyled to 13px/600 Text 1 with the date under it, by opacity alone, nothing ticks and nothing is interpolated; pull to refresh shows the system spinner.
